...
public class Ant {
	 ...
	 * 
	 * @param graph
	 *            the graph that the ant will traverse
	 */
	public Ant(Graph graph) {
		this.graph = graph;
		this.currentWeight = graph.getTotalWeight();
		clear();
	}
	...
	/**
	 * Check if the Ant has made a complete tour around the graph. The number of
	 * vertices in the tour should be one greater than the total vertices in the
	 * graph as the Ant has to come back to its starting position at the end.
	 * 
	 ...
	 * 
	 */
	public static Ant getGreedy(Graph graph) {
		Ant ant = new Ant(graph);
		
		Node current = ant.current;
		while(ant.tour.size() != graph.getTotalVertices()) {
			int min = Integer.MAX_VALUE;
			Edge minEdge = null;
			for(Edge e: ant.graph.getVertex(current)) {
				if (ant.placesTravelled.contains(e))
					continue;
				int temp = (int)Graph.getRequiredEnergy(current, e, ant.currentWeight);
				if (temp < min) {
					minEdge = e;
					min = temp;
				}
			}
			ant.tour.add(minEdge);
			ant.placesTravelled.add(minEdge);
			ant.currentWeight -= minEdge.getWeight();
			current = minEdge;
		}
	...
	public int eval() {
		...
		currentWeight = graph.getTotalWeight();
		for (int i = 1; i < tour.size(); i++) {
			// eval += Graph.getDistance(tour.get(i), tour.get(i-1));
			eval += Graph.getRequiredEnergy(tour.get(i), tour.get(i - 1), currentWeight);
			currentWeight -= tour.get(i).getWeight();
		}
	...
	/**
	 * Get the Edge that the ant should travel to next. Accounts for both the
	 * pheromones and distances of all the edges.
	 * 
	...
	/**
	 * Get the probabilities of all the edges and return them in an array. Example:
	 * if there are 4 edges, each with a probability of 0.25, then the array list
	 * will contain [0.25, 0.50, 0.75, 1.00].
	 * 
	...
	/**
	 * Calculate the denominator for the formula that determines the probability of
	 * an ant moving from the current location to another.
	 * 
	...
	/**
	 * Calculate the pheromone on Edge e, to the power of alpha. Calculate the
	 * desirability of Edge e based on distance, to the power of beta. Get the
	 * product of the two results.
	 * 
	 * @param e
	 *            the Edge to perform the calculations on
	...
	private double desirability(Edge e) {
		...
		double distanceValue = Math.pow(1 / (distance * (currentWeight + 1)), graph.getBeta());
