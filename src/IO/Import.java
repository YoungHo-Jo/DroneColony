...
public class Import {
    ...
    public static Graph getGraph (double evaporationRate, int alpha, int beta) {
        ...
        for (int i = startingLine; i < startingLine+numOfCities; i++) { 
            ...
            int weight = (int)Double.parseDouble(line[1].trim());
            int x = (int)Double.parseDouble(line[2].trim());
            int y = (int)Double.parseDouble(line[3].trim());
            Vertex v = new Vertex(line[0], x, y, weight);
